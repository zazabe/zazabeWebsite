var boxes = function (options){
	...
	this.options = $.extend({
		...
		effect:           'linear',
		duration:         1000,
		incomingEffect:   'linear',
		incomingDuration: 1000,
		interval:         5000,
		boxHeight:        $('#boxes li').innerHeight(),
		boxWidth:         $('#boxes li').innerWidth()
	}, options);
...
boxes.prototype.initBoxesPosition = function(){
	...
	$(this).bind('boxsAnimationStarted', function(e, data){
		console.log(data);
		$(data.element).animate(data.animation, data.parent.options.duration, data.parent.options.effect, function(){
			data.parent.endedAnimation++;
			if(data.parent.endedAnimation >= data.movingBoxs.length){
				$(data.parent).trigger('boxsAnimationFinished',{parent: data.parent, movingBoxs: data.movingBoxs,  hiddenBox: data.hiddenBox});
				data.parent.endedAnimation = 0;
			}
		});
	});
...
boxes.prototype.slideBoxes = function(outsideIndex){
	...
	switch(outsideBoxParams.anim){
		case 'left':
			incomingPosition  = {
					left: outsideBoxParams.pos.fromLeft,
		...
		case 'top':
			incomingPosition  = {
					...
					top:  outsideBoxParams.pos.fromTop
			};
			break;
	}
	...
	hiddenBox.css(incomingPosition);
	
	var p = this;
	$(hiddenBox).animate(incomingAnimation, this.options.incomingDuration, this.options.incomingEffect, function(){
		...
		for(var index=0;index<movingBoxs.length;index++){
			...
			//shit/trick... 
			$(p).trigger('boxsAnimationStarted',{
						parent:     p, 
						movingBoxs: movingBoxs,  
						hiddenBox:  hiddenBox, 
						element:    element,
						animation:  animation
					});
...
boxes.prototype.defineOutsidesPosition = function(){
	...
	this.walkBoxesMatrice(function(line, row, box){
		...
		if(row == 0){
			...
			outside.push({
				pos : {
					fromTop : $(box).position().top - (options.boxHeight * 2),
		...
		if(line == 0){
			...
			outside.push({
				pos : {
					fromLeft : $(box).position().left - (options.boxWidth * 2),
